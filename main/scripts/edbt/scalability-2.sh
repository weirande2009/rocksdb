...
entry_size=1024
run_multiple_times_for_baseline 30 70 $num_workloads $rocksdb_root_dir $num_operation $entry_size $experiment_name &
run_multiple_times_for_baseline 10 90 $num_workloads $rocksdb_root_dir $num_operation $entry_size $experiment_name &
...
experiment_name=160_256_size
rocksdb_root_dir=/scratchNVM1/ranw/$experiment_name
num_operation=$((160 * 1024 * 1024))
entry_size=256
run_multiple_times_for_baseline 50 50 $num_workloads $rocksdb_root_dir $num_operation $entry_size $experiment_name &
run_multiple_times_for_baseline 30 70 $num_workloads $rocksdb_root_dir $num_operation $entry_size $experiment_name &
run_multiple_times_for_baseline 10 90 $num_workloads $rocksdb_root_dir $num_operation $entry_size $experiment_name &
