...
experiment_name=buffer_size_512KB
num_operation=$((220 * 1024))
buffer_size=$((512 * 1024))

run_multiple_times_for_a_type 100 0 0 $num_workloads $rocksdb_root_dir --UD\ 1 $experiment_name $num_operation $buffer_size &
run_multiple_times_for_a_type 90 10 0 $num_workloads $rocksdb_root_dir --UD\ 1 $experiment_name $num_operation $buffer_size &

wait $(jobs -p)

run_multiple_times_for_a_type 80 20 0 $num_workloads $rocksdb_root_dir --UD\ 1 $experiment_name $num_operation $buffer_size &
run_multiple_times_for_a_type 70 30 0 $num_workloads $rocksdb_root_dir --UD\ 1 $experiment_name $num_operation $buffer_size &

wait $(jobs -p)

run_multiple_times_for_a_type 60 40 0 $num_workloads $rocksdb_root_dir --UD\ 1 $experiment_name $num_operation $buffer_size &
run_multiple_times_for_a_type 50 50 0 $num_workloads $rocksdb_root_dir --UD\ 1 $experiment_name $num_operation $buffer_size &
